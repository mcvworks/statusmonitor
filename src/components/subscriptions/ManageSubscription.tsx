...
export function ManageSubscription({ id, token }: { id: string; token: string }) {
  ...
  if (phase === "error") return <div className="glass-card space-y-3 rounded-2xl p-7"><p className="text-sm text-critical-ink" role="alert">{error}</p><Link href="/subscribe/manage" className="text-sm text-primary-ink underline">Request a new management link</Link></div>;
  if (phase === "unsubscribed") return (
    <div className="glass-card space-y-3 rounded-2xl p-5 sm:p-7" role="status">
      ...
      <p className="text-sm text-text-muted">Browser, Slack, and Teams alerts are managed separately in <Link href="/dashboard/settings" className="text-primary-ink underline">alert settings</Link>.</p>
      <Link href="/" className="inline-block text-sm text-primary-ink underline">View live status</Link>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <section id="unsubscribe" className="glass-card space-y-3 rounded-2xl border border-critical/30 p-5 sm:p-7" aria-labelledby="stop-email-heading">
        ...
        <button type="button" onClick={unsubscribe} disabled={busy !== null} className="rounded-lg border border-critical/60 px-4 py-2.5 text-sm font-semibold text-critical-ink hover:bg-critical/10 disabled:opacity-50">{busy === "unsubscribe" ? "Stopping email alerts…" : "Unsubscribe from email alerts"}</button>
        <p className="text-xs leading-5 text-text-muted">This stops email alerts only. Manage browser, Slack, and Teams alerts in <Link href="/dashboard/settings" className="text-primary-ink underline">alert settings</Link>.</p>
      </section>
      ...
      {!confirmed ? <p className="glass-card rounded-2xl p-5 text-sm text-text-secondary">This email subscription has not been confirmed, so alerts have not started. Use your confirmation email to start alerts, or unsubscribe above to remove the pending subscription.</p> : (
        <form onSubmit={save} className="glass-card space-y-5 rounded-2xl p-5 sm:p-7">
          ...
          <fieldset disabled={busy !== null}><legend className="mb-2 text-sm font-medium text-text-primary">Severity</legend><div className="flex flex-wrap gap-2">{SEVERITIES.map((severity) => <button type="button" aria-pressed={severities.includes(severity)} key={severity} onClick={() => toggle(severities, severity, setSeverities)} className={`rounded-full border px-3 py-1.5 text-xs capitalize ${severities.includes(severity) ? "border-primary bg-primary/10 text-primary-ink" : "border-border text-text-secondary"}`}>{severity}</button>)}</div></fieldset>
          <fieldset disabled={busy !== null}><legend className="mb-1 text-sm font-medium text-text-primary">Providers</legend><p className="mb-3 text-xs text-text-muted">Selecting no individual providers means all providers. To stop emails, use Unsubscribe above.</p><div className="max-h-64 overflow-y-auto rounded-xl border border-border p-3"><div className="flex flex-wrap gap-2"><button type="button" aria-pressed={sources.length === 0} onClick={() => setSources([])} className={`rounded-full border px-3 py-1.5 text-xs ${sources.length === 0 ? "border-secondary text-secondary-ink" : "border-border text-text-secondary"}`}>All providers</button>{Object.entries(PROVIDERS).map(([key, provider]) => <button type="button" aria-pressed={sources.includes(key)} key={key} onClick={() => toggle(sources, key, setSources)} className={`rounded-full border px-3 py-1.5 text-xs ${sources.includes(key) ? "border-primary bg-primary/10 text-primary-ink" : "border-border text-text-secondary"}`}>{provider.name}</button>)}</div></div></fieldset>
          {status && <p className="text-sm text-secondary-ink" role="status">{status}</p>}
          <button disabled={busy !== null || severities.length === 0} className="rounded-lg bg-primary px-5 py-2.5 text-sm font-semibold text-on-primary disabled:opacity-50">{busy === "save" ? "Saving…" : "Save preferences"}</button>
        </form>
      )}
    </div>
  );
