  ...
  return (
    <div className="glass-card space-y-5 p-5">
      ...
      {connections.map((connection, index) => (
        <div key={connection.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-secondary/20 bg-secondary/5 px-4 py-3"><span className="flex items-center gap-2 text-xs text-secondary"><Check className="h-4 w-4" />{connection.channel === "slack" ? "Slack" : "Teams"} channel {index + 1} connected</span><button onClick={() => disconnect(connection)} disabled={disconnecting !== null || connecting} aria-label={`Disconnect ${connection.channel === "slack" ? "Slack" : "Teams"} channel ${index + 1}`} className="inline-flex items-center gap-2 rounded-lg border border-critical/30 p-2 text-xs text-critical hover:bg-critical/10 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" />{disconnecting === connection.id ? "Disconnecting…" : `Disconnect ${connection.channel === "slack" ? "Slack" : "Teams"}`}</button></div>
      ))}
      <form onSubmit={connect} className="space-y-4">
        ...
        <button disabled={connecting || disconnecting !== null || severities.length === 0} className="rounded-lg bg-primary px-4 py-2 text-xs font-semibold text-background disabled:opacity-50">{connecting ? "Connecting…" : `Connect ${channel === "slack" ? "Slack" : "Teams"} channel`}</button>
      </form>
