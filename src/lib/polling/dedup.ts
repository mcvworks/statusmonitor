...
// Each source/externalId pair uses two parameters and an OR branch. Keep
// queries below SQLite's parameter and expression-depth limits for large feeds.
...
export async function deduplicateAlerts(
  ...
  for (let index = 0; index < incoming.length; index += LOOKUP_BATCH_SIZE) {
    const batch = incoming.slice(index, index + LOOKUP_BATCH_SIZE);
    const existing = await prisma.alert.findMany({
      where: {
        OR: batch.map((a) => ({ source: a.source, externalId: a.externalId })),
      },
      select: {
        source: true,
        externalId: true,
